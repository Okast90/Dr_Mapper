...
class DroneMappingEngine {
  ...
  static double _estimatePolygonMainAxisAngle(List<Point> polygon) {
    if (polygon.length < 3) return 0.0;

    final centroid = Point(
      polygon.map((p) => p.x).reduce((a, b) => a + b) / polygon.length,
      polygon.map((p) => p.y).reduce((a, b) => a + b) / polygon.length,
    );
  ...
  List<LatLng> generateWaypoints(List<LatLng> polygon, bool createCameraPoints, [bool fillGrid = false, LatLng? homePoint]) {
    ...
    var localPolygon = _latLngToMeters(polygon);
    final shouldUseHorizontalSweep = _preferHorizontalSweep(localPolygon);
    final effectiveAngle = angle == 0 ? _estimatePolygonMainAxisAngle(localPolygon) : angle;
    var rotatedPolygon = _rotatePolygon(localPolygon, effectiveAngle);
    var origin = polygon[0];
    ...
    if (homePoint == null) {
      ...
      var rotatedWaypointsBack = _rotatePolygon(waypoints, -effectiveAngle);
      ...
      if (!fillGrid) {
        if (shouldUseHorizontalSweep) {
          ...
          if (horiz.isEmpty) {
            var vert = _generateVerticalWaypoints(rotatedPolygon, createCameraPoints, homeP, minX, maxX, minY, maxY);
            return _metersToLatLng(_rotatePolygon(vert, -effectiveAngle), origin);
          }
          ...
          if (vert.isEmpty) {
            var horiz = _generateHorizontalWaypoints(rotatedPolygon, createCameraPoints, homeP, minX, maxX, minY, maxY);
            if (horiz.isEmpty) return [];
            return _metersToLatLng(_rotatePolygon(horiz, -effectiveAngle), origin);
          }
          var vertRev = vert.reversed.toList();
          double distVert = _distance(vert.last, homeP);
          double distVertRev = _distance(vertRev.last, homeP);
          selected = distVert < distVertRev ? vert : vertRev;
        }
      } else {
        // Horizontal first
        var horiz = _generateHorizontalWaypoints(rotatedPolygon, createCameraPoints, homeP, minX, maxX, minY, maxY);
        if (horiz.isEmpty) {
          var fallbackVertical = _generateVerticalWaypoints(rotatedPolygon, createCameraPoints, homeP, minX, maxX, minY, maxY);
          if (fallbackVertical.isEmpty) {
            return [];
          }
          return _metersToLatLng(_rotatePolygon(fallbackVertical, -angle), origin);
        }
        ...
        if (vertF.isEmpty) {
          return _metersToLatLng(_rotatePolygon(horiz, -angle), origin);
        }
        ...
        var best = validPaths.first;
        var bestDist = _distance(best.last, homeP);
        for (var p in validPaths.skip(1)) {
      ...
      var rotatedWaypointsBack = _rotatePolygon(selected, -effectiveAngle);
      return _metersToLatLng(rotatedWaypointsBack, origin);
    }
  }
