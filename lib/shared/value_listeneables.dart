...
class ValueListenables extends ChangeNotifier {
  ...
  set altitude(int value) {
    final safeValue = value.clamp(minAltitude, maxAltitude);
    if (_altitude.value == safeValue) {
      return;
    }
    _altitude.value = safeValue;
    if (_groundOffset.value >= _altitude.value) {
      _groundOffset.value = _altitude.value - 1;
    }
    notifyListeners();
  }
  ...
  set groundOffset(int value) {
    final maxSafeGroundOffset = (_altitude.value - 1).clamp(0, maxAltitude - 1);
    final safeValue = value.clamp(0, maxSafeGroundOffset);
    if (_groundOffset.value == safeValue) {
      return;
    }
    _groundOffset.value = safeValue;
    notifyListeners();
  }
